<script lang="ts">
	...
	const base = 'inline-flex items-center justify-center gap-2 text-xs font-mono no-underline transition-colors cursor-pointer border';
	...
	const cls = $derived(`${base} ${sizes[size]} ${variants[variant]} ${block ? 'w-full' : ''} ${className}`);
</script>
